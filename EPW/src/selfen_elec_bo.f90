...
sigmai_chk(nbndsub, nksf)  !! extra
  ...
  real(kind=DP) :: weight_chk(nxqf, 2*nmodes, ibndmax-ibndmin+1, ibndmax-ibndmin+1)
  !
				...
				DO  jbnd = 1, ibndmax-ibndmin+1
					...
					DO ibnd = 1, ibndmax-ibndmin+1
						...
						ELSE
							g2 = 0.d0
						ENDIF
            weight_chk( iq, imode, ibnd, jbnd) = wqf(iq) * real (          &
                   ( (       wgkq + wgq ) / ( ekk - ( ekq - wq ) - ci * degaussw )  +  &
                     ( one - wgkq + wgq ) / ( ekk - ( ekq + wq ) - ci * degaussw ) ) )
            weight_chk( iq, nmodes+imode, ibnd, jbnd) = wqf(iq) * real (   &
                   ( (       wgkq + wgq ) / ( ekk - ( ekq - wq ) - ci * degaussw )  +  &
                     ( one - wgkq + wgq ) / ( ekk - ( ekq + wq ) - ci * degaussw ) ) )
			...
			DO itet= 1, ntet
				DO jtet = 1, ncorn
					DO imode = 1, 2*nmodes
					    ...
					    IF ( tet(itet)%p(jtet)%c(jbnd) .gt. 1.0d-8 .or.weight_chk(tet(itet)%p(jtet)%i, imode, ibnd,jbnd) .gt. 1.0d-8) THEN
              WRITE(6,'(/5x," itet: ", i5," jbnd: ", i5," wt: ", f14.10, " weight = ", f14.10)') itet,jbnd, tet(itet)%p(jtet)%c(imode),weight_chk( tet(itet)%p(jtet)%i, imode, ibnd, jbnd)
              ENDIF
          ENDDO ! imode
				ENDDO ! jtet
			ENDDO ! itet
